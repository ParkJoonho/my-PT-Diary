...
| 기준일 | 주간 범위와 스트릭을 계산하는 날짜. 요청값이 없으면 서버의 현재 날짜 사용 |
...
| WT-004 | 완료 건수 | 같은 날짜의 여러 운동 완료 기록은 별도 건수로 유지한다. | 날짜별 `COUNT(*)` 결과를 `days[].completionCount`로 반환한다. 화면은 현재 건수를 별도로 노출하지 않는다. | 일부 구현 | `server/src/modules/weekly-tracker/weekly-tracker.repository.ts`, `client/src/features/home/types/routine.ts` |
...
| WT-013 | 한국 날짜 기준 | 서비스 정책상 한국 시간 기준의 오늘 날짜를 일관되게 사용해야 한다. | 서버의 로컬 타임존에서 `new Date()`와 `getFullYear/getMonth/getDate`를 사용한다. `Asia/Seoul`을 명시하지 않아 UTC 등 다른 타임존에 배포하면 날짜 경계가 달라질 수 있다. | 일부 구현 | `server/src/modules/weekly-tracker/weekly-tracker.service.ts` |
...
| WT-017 | 캐시·재시도 | 불필요한 중복 조회를 줄이고 일시적 실패를 재시도한다. | React Query의 `staleTime`은 30초이고 실패 시 1회 재시도한다. 사용자 키는 쿼리 키에 포함되지 않는다. | 일부 구현 | `client/src/shared/api/query-client.ts`, `client/src/shared/api/weekly-tracker.ts` |
| WT-018 | 운동 완료 생성 연동 | 실제 운동 완료 시 해당 날짜와 출처를 서버에 저장하고 주간 요약을 갱신해야 한다. | 서버 생성 API와 Orval 생성 함수는 존재하지만 현재 클라이언트 기능에서 호출하거나 주간 쿼리를 무효화하는 흐름은 없다. | 미구현 | `server/src/modules/weekly-tracker/weekly-tracker.controller.ts`, `client/src/shared/api/generated/` |
...
현재 확인된 전체 결과는 클라이언트 5개, 서버 단위·통합 10개, 서버 E2E 4개로 총 19개 테스트 통과다. 클라이언트와 서버 TypeScript 검사도 통과했다.
...
| --- | --- | --- | --- |
| WT-GAP-001 | 운동 완료 저장 경로 미연결 | 앱에서 운동을 완료해도 `workout_completions`에 자동 반영되지 않는다. | 실제 운동 종료·PT 기록·야외운동 저장 흐름과 POST API 연결 |
| WT-GAP-002 | 한국 시간대 미명시 | UTC 서버에서는 자정 전후 기준일이 한국 날짜와 다를 수 있다. | `Asia/Seoul` 정책 확정 후 날짜 계산 테스트 추가 |
...
| WT-GAP-006 | 쿼리 키에 사용자 미포함 | 런타임 중 사용자 키가 바뀌면 이전 사용자의 캐시를 재사용할 가능성이 있다. | 사용자 키를 쿼리 키에 포함하고 키 변경 시 캐시 정리 |
