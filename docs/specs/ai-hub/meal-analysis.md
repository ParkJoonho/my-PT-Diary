...
| MEAL-012 | 전·후 사진의 카메라와 앨범은 앱인토스 공식 권한 및 공개 SDK로 열어야 한다. | Granite manifest에 `camera/access`, `photos/read`를 선언하고 공통 picker가 권한 확인·요청 후 `openCamera`, `fetchAlbumPhotos`를 호출한다. | O | [현재 사진 선택기][cur-picker], [사진 선택 테스트][cur-picker-test] |
...
| 사진 입력 | 원본 전·후 2열 사진 카드와 앱인토스 카메라·앨범 공통 picker 연결 | [사진 입력 UI][cur-photo], [현재 사진 선택기][cur-picker] |
...
| 이미지 picker·체형·식단 화면 테스트 | 3개 suite, 15개 test 통과 |
| 클라이언트 전체 테스트 | 62개 suite, 179개 test 통과 |
| 타입 검사 | `npm run typecheck` 통과 |
| 정적 검사 | 식단·AI Hub 변경 파일 Biome 통과 |
| 프로덕션 빌드 | RN 0.84.0·0.72.6의 iOS·Android 번들 0 error, 0 warning |
...
[cur-picker-test]: ../../../client/src/features/body-analysis/lib/__tests__/pick-image.test.ts
