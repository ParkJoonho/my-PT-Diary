...
| AIHUB-002 | Hub | 원본의 6개 기능명·요약·아이콘·화살표 카드를 정적 목록으로 표시한다. | O | X | 서버 작업 불필요. 추가 카드와 미구현 배지는 제거되어 6개 구성은 복원됐지만, `AI 자세 분석`이 원본의 accessibility 아이콘 대신 체형 분석과 같은 `ScanFace`를 사용한다. 카드 구성 요소 전체가 일치하지 않아 엄격한 원본 디자인 기준으로 X다. [원본 Hub][src-hub] [현재 Hub][cur-hub] |
...
| AIHUB-009 | 체형 | 체형 타입, 신체 비율, 상·하체 특징, 자세 점수를 검증된 계약으로 받고 원본 결과 UI로 표시한다. | O | X | 서버 계약과 BODY MBTI·비율 바·상하체·자세 카드 구조는 복원됐다. 그러나 현재 공통 비율 컴포넌트가 두 비율 모두 `값 × 100` 위치를 사용해, 원본에서 `값 × 50`으로 중앙에 놓던 상하체 평균 1.0 마커가 오른쪽 끝에 표시된다. 핵심 결과 시각화가 잘못되어 X다. [원본 체형 결과][src-body-ui] [현재 체형 스키마][cur-body-schema] [현재 체형 결과][cur-body-result] |
...
| AIHUB-012 | 체형 | 분석 결과를 사용자별로 자동 저장하고 분석 성공과 저장 실패를 구분하며 재시도할 수 있다. | O | X | 자동 저장과 `recordSave.saved/failed` 분리는 유지되고, 저장 실패 배너에서 AI 분석을 다시 실행하지 않고 기록 생성 POST만 호출하는 UI도 추가됐다. 다만 클라이언트가 Orval 계약을 재생성하지 않고 직접 `fetch`와 수기 타입을 추가했으며, 실패 응답 뒤 실제 저장됐을 가능성에 대비한 멱등 키가 없어 재시도 시 중복 기록을 만들 수 있다. 재시도 클라이언트 테스트도 없다. [현재 체형 서비스][cur-body-service] [현재 저장 배너][cur-save-banner] [현재 기록 API][cur-create-record-api] [현재 이력 컨트롤러][cur-history-controller] |
| AIHUB-013 | 체형 | 자신의 체형 이력을 최신순으로 조회하고 같은 `body` 타입 두 건만 선택해 AI 비교한다. | O | X | 원본의 `전체/체형/자세/통합` 필터, `body` 기록만 선택하는 비교 제한, 전체 변화·체형 변화·개선·주의·자세·정량·추천·격려 결과는 복원됐다. 그러나 원본의 뒤로가기·분석 타입 아이콘을 텍스트 UI로 바꿨고, 자세 변화의 `개선/유지/악화` 방향 아이콘도 단순 `→`로 축약했다. 기능 흐름은 대부분 복원됐지만 엄격한 원본 디자인/정보 표현 기준으로 X다. [원본 이력][src-history] [현재 이력 서버][cur-history-server] [현재 이력 화면][cur-history-ui] [현재 이력 비교 결과][cur-history-comparison] |
...
| AIHUB-043 | 신발 | Hub의 신발 추천 카드에서 보행·신발 추천 기능으로 진입한다. | O | O | 서버 작업 불필요. Hub의 신발 추천 카드는 `/ai-analysis`를 신발 진입 모드로 열고, 화면 제목·안내를 신발 추천으로 바꾼 뒤 신발 밑창 입력 섹션으로 자동 이동한다. 신발만으로 분석하지 못하는 문제는 AIHUB-044에서 별도 X다. [현재 Hub][cur-hub] [현재 체형 화면][cur-body-ui] [현재 신발 진입 상태][cur-body-entry-store] |
...
| AIHUB-053 | 신발 | 사용자별 신발 결과를 저장하고 이력에서 마모·보행·사이즈·추천 전체를 다시 본다. | O | X | 서버는 전체 결과를 사용자별 `body` raw result로 저장하고, 상세 화면도 메인 결과 재사용 대신 원본 이력 전용 구조로 분리됐다. 그러나 현재 이력 상세는 마모 패턴·보행 유형·보행 추천만 표시하고 발 정렬, 신체 영향, 사이즈, 매칭 로직, `현재/교정 후 × 일상/운동` 신발 추천을 모두 생략한다. 전체 재열람 요구를 충족하지 못해 X다. [현재 체형 서비스][cur-body-service] [원본 이력][src-history] [현재 이력 상세][cur-history-detail] |
...
| AICODE-001 | Hub 코드 대조 | 원본 Hub의 카드 구성은 정확히 6개이고 별도 `나의 몸매 & 스타일` 카드는 없다. | O | O | `나의 몸매 & 스타일` 카드와 미구현 배지를 제거해 원본과 같은 6개 카드 구성으로 복원했다. 자세 아이콘 불일치는 더 넓은 카드 구성 요구인 AIHUB-002에서 별도 X다. [원본 Hub][src-hub] [현재 Hub][cur-hub] |
...
- 현재 판정 합계는 백엔드 `O 20 / X 51`, 클라이언트 `O 13 / X 58`이다.
...
- 체형 분석도 완료 판정은 아니다. PT 컨텍스트, 촬영일, 현재 체성분, 상하체 비율 마커, 최신 연구, 멱등한 저장 재시도, 이력 화면 원본 UX와 이력의 신발 전체 재열람이 남아 있다.
- 아테나, 식단 분석, 영상 자세 분석, 통합 피트니스 분석은 카드 문구만 있고 백엔드와 실제 화면은 마이그레이션되지 않았다.
- 신발 추천은 Hub 진입과 서버 계약, 메인 결과 UI가 들어와 있지만 신발만의 입력 흐름, 판독 검증, 실측, 민감 속성 처리, 상품 현재성, 의료 안전 경계와 이력의 전체 재열람이 없어 기능 전체로는 완료가 아니다.
- Hub는 원본과 같은 6개 카드로 복원됐지만 자세 분석 아이콘이 원본과 달라 AIHUB-002는 아직 X다.
...
- 재검증: 클라이언트 타입체크 및 AI Hub/body-analysis 대상 테스트 8개 suite/17개 test 통과
- 재검증: 서버 타입체크 및 analysis-records 대상 테스트 3개 suite/12개 test 통과
- 정적 검사: 변경 파일 대상 Biome는 30개 오류, ESLint는 5개 오류가 남아 통과하지 못했다. 타입체크·테스트 통과만으로 완료 판정하지 않았다.
- 테스트 한계: 추가된 UI 테스트는 주로 섹션 제목 존재 여부를 검증해 비율 마커 계산, Hub 신발 클릭·자동 스크롤, 신발 이력 전체 필드 재열람은 보장하지 않는다.
