...
| AIHUB-002 | Hub | 원본의 6개 기능명·요약·아이콘·화살표 카드를 정적 목록으로 표시한다. | O | O | 서버 작업 불필요. 원본과 같은 6개 카드 구성·아이콘·문구를 유지하고, 아직 제공하지 않는 아테나·자세·통합 분석에만 작은 `미구현` 뱃지를 표시한다. [원본 Hub][src-hub] [현재 Hub][cur-hub] |
| AIHUB-003 | Hub | 카드를 누르면 실제 화면으로 이동하고, 미제공 기능이면 막힌 경로 대신 상태를 알린다. | O | O | 서버 작업 불필요. 체형·식단·신발은 실제 대응 화면으로 이동하고, 아테나·자세·통합 분석은 잘못된 경로로 이동하거나 알림을 띄우지 않고 카드의 `미구현` 뱃지로 상태를 알린다. [현재 Hub][cur-hub] [현재 라우트][cur-routes] |
...
| AIHUB-024 | 식단 | 사진→영양 분석→확인 후 저장→당일 합계→AI 가이드의 전체 흐름을 제공한다. | O | O | 서버의 분석·기록·조회·합계·가이드 API를 Orval로 생성해 원본의 `식단 분석/식단 가이드` 2탭 화면에 연결했다. 분석 결과를 확인한 뒤에만 저장하고 저장 후 오늘 기록과 합계를 갱신한다. [현재 식단 컨트롤러][cur-meal-controller] [현재 식단 API][cur-meal-client-api] [현재 식단 화면][cur-meal-ui] |
| AIHUB-025 | 식단 | Hub 카드에서 식단 분석으로 진입하고 분석·저장·조회는 현재 사용자 기록만 다룬다. | O | O | Hub 카드가 `/meal-analysis`로 이동하며 모든 식단 API가 필수 `x-user-key`를 사용한다. 기록 Suspense Query도 사용자 키와 날짜를 query key에 포함한다. [현재 Hub][cur-hub] [현재 식단 API][cur-meal-client-api] [현재 식단 저장소][cur-meal-repo] |
| AIHUB-026 | 식단 | 아침/점심/간식/저녁을 선택하고 식사 전 사진은 필수, 식사 후 사진은 선택으로 받는다. | O | O | 원본의 네 식사 유형 선택과 2열 전·후 사진 카드를 복원했다. 식사 전 사진 없이는 버튼이 비활성화되고 서버도 네 유형 enum과 필수·선택 사진을 검증한다. [현재 식단 화면][cur-meal-ui] [현재 식단 사진 입력][cur-meal-photo] [현재 식단 스키마][cur-meal-schema] |
| AIHUB-027 | 식단 | 전·후 사진이 있으면 남긴 양과 신뢰 가능한 촬영 시간 차이로 섭취량·식사 속도를 추정한다. | O | X | 서버와 화면은 전·후 사진 분석과 음식별 섭취율 결과를 지원한다. 다만 Apps in Toss 카메라·앨범 응답에 EXIF/촬영 시각이 없어 원본의 자동 식사 시간 계산을 연결하지 못했고 해당 문구 옆에 `미구현` 뱃지를 표시했다. EXIF 신뢰성 보정은 원본에 없는 `(QA 기대사항)`이다. [현재 식단 AI][cur-meal-ai] [현재 식단 사진 입력][cur-meal-photo] [현재 사진 선택][cur-pick-image] |
| AIHUB-028 | 식단 | 음식별 중량·칼로리·탄단지·식이섬유·나트륨과 합계·균형·조언을 검증해 반환한다. | O | O | 서버가 음식별 추정량·섭취율·영양소와 합계·균형·조언·운동 상쇄량·요약을 검증하고, 화면이 원본 결과 섹션 순서로 표시한다. 외부 상품·칼로리 DB 사실성 검증은 원본에 없는 `(QA 기대사항)`이라 추가하지 않았다. [현재 식단 스키마][cur-meal-schema] [현재 식단 결과][cur-meal-result] |
| AIHUB-029 | 식단 | 총열량·영양소·인식 음식·AI 조언을 보고 저장 또는 새 사진 재분석을 선택한다. | O | O | 총열량·탄단지·균형·인식 음식·운동 상쇄량·조언·식사 속도·요약을 표시한 뒤 `기록 저장`과 `다시 분석`을 원본처럼 분리했다. [현재 식단 결과][cur-meal-result] [현재 식단 상태][cur-meal-store] |
| AIHUB-030 | 식단 | 사용자가 저장을 선택하면 사용자·식사유형·현지 식사일·분석 결과·영양 합계를 저장한다. | O | O | 저장 버튼이 현재 분석 결과·선택 식사 유형·클라이언트 현지 날짜를 별도 저장 API로 보내고 서버가 `meal_records`에 사용자별로 저장한다. [현재 식단 화면][cur-meal-ui] [현재 식단 API][cur-meal-client-api] [현재 식단 저장소][cur-meal-repo] |
| AIHUB-031 | 식단 | 현지 당일 기록의 열량·주요 영양소·끼니 수를 합산하고 각 끼니 요약을 재확인한다. | O | O | 화면 상단에 오늘의 열량·탄단지·끼니 수를, 하단에 유형별 열량·탄단지 기록을 원본 카드 구조로 표시한다. 저장 성공 시 두 Suspense Query를 함께 갱신한다. [현재 오늘 식단][cur-meal-today] [현재 식단 API][cur-meal-client-api] |
| AIHUB-032 | 식단 | 식사일 저장·당일 조회·합계에 회원 현지 날짜를 일관되게 쓴다. | O | O | 클라이언트가 `getClientTodayDate()`로 현지 `YYYY-MM-DD`를 만들고 같은 값을 기록·합계 조회, 저장, 가이드 생성에 사용한다. 서버도 해당 날짜를 변환 없이 저장·필터한다. [현재 식단 화면][cur-meal-ui] [현재 날짜 도구][cur-date] [현재 식단 저장소][cur-meal-repo] |
| AIHUB-033 | 식단 | 식사 유형·날짜·영양 수치의 enum·형식·범위를 서버에서 검증한다. | O | O | 화면은 네 식사 유형만 선택하게 하고 필수 사진 없이는 분석을 막는다. 서버는 날짜·사진 크기·식사 시간과 음식·영양·점수·비율 범위를 요청 및 AI 응답 양쪽에서 검증한다. [현재 식단 화면][cur-meal-ui] [현재 식단 스키마][cur-meal-schema] |
| AIHUB-034 | 식단 | 저장된 식단으로 섭취 평가와 권장 식사 구성을 만들고 기록이 없으면 일반 안내임을 구분한다. | O | O | 서버가 현재 사용자·날짜의 저장 기록만 사용하고 `sourceMealCount`를 반환한다. 화면도 기록이 있으면 맞춤 가이드, 없으면 미리 보기 문구로 구분한다. [현재 식단 서비스][cur-meal-service] [현재 식단 가이드][cur-meal-guide] |
| AIHUB-035 | 식단 | 가이드의 평가·목표 영양소·식사 구성·개선 조언 계약을 화면과 일치시킨다. | O | O | 서버와 클라이언트가 `overallAssessment`, `macroTargets`, `mealPlan`, `tips` 계약을 공유하며 화면이 평가·목표·추천 식사·팁과 다시 생성을 모두 표시한다. [현재 식단 응답 DTO][cur-meal-response] [현재 식단 가이드][cur-meal-guide] |
...
| AICODE-001 | Hub 코드 대조 | 원본 Hub의 카드 구성은 정확히 6개이고 별도 `나의 몸매 & 스타일` 카드는 없다. | O | O | `나의 몸매 & 스타일` 카드를 제거해 원본과 같은 6개 카드·아이콘 구성으로 복원했다. 현재 미제공인 아테나·자세·통합 분석에만 공용 `미구현` 뱃지를 표시한다. [원본 Hub][src-hub] [현재 Hub][cur-hub] |
...
- 현재 판정 합계는 백엔드 `O 34 / X 37`, 클라이언트 `O 31 / X 40`이다.
- 현재 실제로 끝까지 연결된 범위는 `AI Hub → 체형 분석`, `AI Hub → 식단 분석`, `AI Hub → 신발 입력 섹션`, 사진 분석 요청, 사용자별 저장, 전·후 비교의 핵심 결과, 다각도·의료 상세, 식단 기록·일일 합계·AI 가이드, 그리고 체형 분석에 포함된 신발 결과의 메인 화면 렌더링이다.
...
- 식단 분석은 원본의 2탭 화면, 전·후 사진 분석, 확인 후 저장, 사용자별 날짜 조회와 일일 합계, 저장 기록 기반 가이드까지 연결됐다. Apps in Toss 사진 API가 촬영 시각을 제공하지 않아 EXIF 기반 식사 시간 자동 계산만 `미구현`으로 남아 있다.
...
- Hub는 원본과 같은 6개 카드와 자세 분석 아이콘 구성을 유지하며, 식단 카드는 실제 화면으로 연결되고 나머지 미제공 기능에는 `미구현` 뱃지를 표시한다.

## 확인 범위

- 원본 클라이언트: Hub, 체형/신발/영상 자세/전·후 비교, 이력, 아테나, 식단, 통합 분석, 활성 운동
- 원본 서버: AI 분석·채팅·식단 가이드·상태 벡터·기록 저장/비교 경로
- 현재 클라이언트: 생성 라우트 전체, Hub, 체형 입력/결과, 전·후 비교, 이력, 식단 분석·기록·가이드, 활성 운동
...
- 재검증: 클라이언트 전체 테스트 32개 suite/88개 test, 서버 전체 테스트 27개 suite/106개 test 통과
- 정적 검사: 클라이언트 변경 기능 범위 Biome와 서버 변경 파일 ESLint 통과
- 클라이언트 빌드: iOS·Android 프로덕션 번들 0 error, 0 warning
- 실제 API 확인: 같은 멱등 키로 분석 기록 생성 POST를 두 번 호출해 동일 기록 ID와 단일 저장 행을 확인했고, 식단 기록 생성·사용자별 날짜 조회·일일 합계와 스모크 기록 정리까지 확인
- 테스트 보강: 비율 마커 계산, Hub 카드·식단 진입, 비교 결과 방향 표현, 신발 이력 전체 재열람과 식단 분석·기록·가이드 계약을 회귀 테스트로 확인
...
[cur-meal-client-api]: ../../../../client/src/features/meal-analysis/api/meal-analysis.ts
[cur-meal-ui]: ../../../../client/src/features/meal-analysis/components/meal-analysis-screen.tsx
[cur-meal-photo]: ../../../../client/src/features/meal-analysis/components/meal-photo-section.tsx
[cur-meal-result]: ../../../../client/src/features/meal-analysis/components/meal-analysis-result.tsx
[cur-meal-today]: ../../../../client/src/features/meal-analysis/components/meal-today-sections.tsx
[cur-meal-guide]: ../../../../client/src/features/meal-analysis/components/diet-guide-tab.tsx
[cur-meal-store]: ../../../../client/src/features/meal-analysis/stores/use-meal-analysis-store.ts
[cur-date]: ../../../../client/src/shared/lib/date.ts
